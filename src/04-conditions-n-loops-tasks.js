 ...
 *   'entente' => null
 */
function findFirstSingleChar(str, index = 0, charCount = new Map()) {
  if (index >= str.length) {
  ...
  return findFirstSingleChar(str, index + 1, charCount);
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (bracketsMap[char]) {
      ...
      if (topOfStack !== bracketsMap[char]) {
        return false;
      }
    } else {
      stack.push(char);
...
function getMatrixProduct(m1, m2) {
  ...
  if (cols1 !== rows2) {
    throw new Error('Invalid matrix dimensions. Columns of the first matrix must match rows of the second matrix.');
  }
